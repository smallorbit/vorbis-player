...
import { type TAnalyzerInputControl, type FreqBinInfo, type OctaveBandMode, type EnergyMeasure } from "../types";
...
export default class FFTAnalyzer implements TAnalyzerInputControl {
  ...
  private _analyze(): void {
    ...
    this._runId = requestAnimationFrame(() => this._analyze());
  }
  ...
  disconnectInputs(): void {
    // Create a copy of the array to iterate over since we'll be modifying the original
    const sourcesCopy = this._sources.slice();
    for (let i = 0; i < sourcesCopy.length; i++) {
      const node = sourcesCopy[i];
