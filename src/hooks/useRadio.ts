 ...
 * for the Dropbox provider. Handles queue generation and integration
 * with the existing playback system.
 */
...
export interface UseRadioReturn {
  ...
  stopRadio: () => void;
  /** Whether the radio feature is available (API key configured + Dropbox provider). */
...
export function useRadio(): UseRadioReturn {
  ...
  const startRadio = useCallback(async (
    ...
    try {
      ...
      // Check if this generation is still the most recent
