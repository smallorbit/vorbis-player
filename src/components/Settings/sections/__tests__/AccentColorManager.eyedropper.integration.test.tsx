 ...
 * `EyedropperOverlay` (not a mock) wrapped in the **real** `Settings`
 ...
 *
 * The fix in `Settings.tsx` (`onPointerDownOutside` +
 ...
 *
 * `SettingsContent` is mocked so this test can render `<AccentColorManager />`
...
vi.mock('../../SettingsMobileTakeover', () => ({
  SettingsMobileTakeover: () => null,
}));

vi.mock('../../SettingsContent', async () => {
  const { AccentColorManager } = await import('../appearance/AccentColorManager');
  return {
    SettingsContent: () => <AccentColorManager />,
  };
});
...
describe('Settings + real EyedropperOverlay (issue #1467)', () => {
  beforeAll(() => {
    /**
  ...
  it('keeps the Settings Dialog open when pointerdown fires inside the eyedropper portal', async () => {
    // #given — real Settings Dialog open with AccentColorManager mounted
    const onClose = vi.fn();
    render(
      <Wrapper>
  ...
  it('keeps the eyedropper portal interactive (pointer-events: auto) so the X close button is not blocked by Radix body pointer-events lockout', async () => {
    // #given — real Settings Dialog open with the eyedropper portal mounted.
