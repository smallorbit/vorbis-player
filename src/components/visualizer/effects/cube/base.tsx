      ...
      for (let col = 0; col < nPerSide; col++) {
        for (let depth = 0; depth < nPerSide; depth++) {
          ...
          if (normCubeX == 0 || normCubeX == 1) {
            normRadialOffset =
              Math.sqrt((normCubeY - 0.5) * (normCubeY - 0.5) + (normCubeZ - 0.5) * (normCubeZ - 0.5)) /
              HALF_DIAGONAL_UNIT_SQUARE;
          } else if (normCubeY == 0 || normCubeY == 1) {
            normRadialOffset =
              Math.sqrt((normCubeX - 0.5) * (normCubeX - 0.5) + (normCubeZ - 0.5) * (normCubeZ - 0.5)) /
              HALF_DIAGONAL_UNIT_SQUARE;
          } else if (normCubeZ == 0 || normCubeZ == 1) {
            normRadialOffset =
              Math.sqrt((normCubeX - 0.5) * (normCubeX - 0.5) + (normCubeY - 0.5) * (normCubeY - 0.5)) /
              HALF_DIAGONAL_UNIT_SQUARE;
          } else {
            // interior
            normRadialOffset = 0;
