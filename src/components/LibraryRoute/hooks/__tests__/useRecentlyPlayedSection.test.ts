...
import { makeLibrarySyncResult, makeRecentlyPlayedResult } from '@/test/fixtures';
...
describe('useRecentlyPlayedSection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockUseRecentlyPlayed.mockReturnValue(makeRecentlyPlayedResult());
  ...
  describe('history pass-through', () => {
    it('returns items matching history entries', () => {
      ...
      mockUseRecentlyPlayed.mockReturnValue(makeRecentlyPlayedResult({ history: [entry] }));
    ...
    it('preserves existing imageUrl from history entry', () => {
      ...
      // #then
      expect(defined(result.current.items[0]).imageUrl).toBe('https://cached.example/img.jpg');
    });
  });
  ...
  describe('imageUrl hydration', () => {
    it('hydrates missing imageUrl from matching playlist in library', () => {
      ...
      // #then
      expect(defined(result.current.items[0]).imageUrl).toBe('https://library.example/img.jpg');
    });
    ...
    it('hydrates missing imageUrl from matching album in library', () => {
      ...
      // #then
      expect(defined(result.current.items[0]).imageUrl).toBe('https://album.example/img.jpg');
    });
    ...
    it('leaves imageUrl undefined when no library match found', () => {
      ...
      // #then
      expect(defined(result.current.items[0]).imageUrl).toBeUndefined();
    });
