...
- **Hydrate without autoplay** — `handleHydrate(session)` in `usePlayerLogic` (`src/hooks/usePlayerLogic.ts`) restores `tracks` / `originalTracks` / `mediaTracksRef`, sets `currentTrackIndex` and `selectedPlaylistId`, applies `lastSession.playbackPosition` to the seek bar, and calls `prepareTrack(track, { positionMs })` on the driving provider — but does NOT call `play()`. A `hydratedPendingPlayRef` records the target index + position so the next user-initiated `handlePlay` starts at the saved offset; `handleNext` / `handlePrevious` / a fresh `playTrack` clear the ref. Distinct from `handleResume` (in `AudioPlayer.tsx`), which autoplays from the saved position.
