...
- **Zen mode transition orchestration** — entry (normal → zen) and exit (zen → normal) are choreographed so the album art stays visually anchored while controls and bottom bar appear/disappear. Timing constants live in `src/constants/zenAnimation.ts` (see the header block comment for the full phase contract):
  - **Entry (~1300ms)**: `ZenControlsWrapper` fades in-place over `ZEN_CONTROLS_DURATION` (300ms) with `grid-template-rows` held at `1fr` so layout height is stable. After `ZEN_ART_ENTER_DELAY` (300ms), the row collapses in parallel with `PlayerStack`'s `max-width` growth and `ContentWrapper`'s `margin-bottom` shrink to 0, all over `ZEN_ART_DURATION` (1000ms). BottomBar auto-hides via `AUTOHIDE_DELAY` in `BottomBar/index.tsx`.
  - **Exit (~1300ms)**: album art shrinks alone for `ZEN_ART_DURATION` (1000ms) — `PlayerStack` `max-width`, `ContentWrapper` `width`. Controls row stays collapsed, BottomBar stays hidden. After `ZEN_EXIT_REENTRY_DELAY` (= `ZEN_ART_DURATION`), controls row expands, opacity/transform restore, `ContentWrapper` `margin-bottom` grows to `BOTTOM_BAR_HEIGHT`, and `BottomBar` slides in — all over `ZEN_CONTROLS_DURATION` / `ZEN_BAR_DURATION`.
  - **Scope gate**: `PlayerStack`'s `max-width` transition is gated by a short-lived `$zenTransitioning` flag raised around `handleZenModeToggle` in `PlayerContent/index.tsx`, so viewport-driven resizes (desktop window resize, iOS address-bar `dvh` changes) stay instantaneous. `BottomBar/index.tsx` watches the `zenModeEnabled` true → false edge and raises `$zenExiting` for `ZEN_EXIT_REENTRY_DELAY` so the bar's slide-in is delayed only on zen exit; normal hover/touch reveals use zero delay.
  - **Reduced motion**: `@media (prefers-reduced-motion: reduce)` collapses transitions to `none` on `PlayerStack`, `ZenControlsWrapper`, and `BottomBarContainer`. `ContentWrapper`'s `width` and `margin-bottom` transitions are intentionally not gated and still animate under reduced motion.
